...
function pleiadesmoon_setup() {
	...
	add_image_size('frontpage-blog', 300, 200, true);
...
function pleiadesmoon_excerpt_more($link) {
	...
	$link = sprintf('<p class="link-more"><a href="%1$s">%2$s</a></p>',
		esc_url(get_permalink( get_the_ID())),
		/* translators: %s: Name of current post */
		sprintf(__('Leer más<span class="screen-reader-text"> "%s"</span>', 'pleiadesmoon'), get_the_title( get_the_ID()))
	);
	return ' &hellip; ' . $link;
}
...
function pleiadesmoon_scripts() {
	...
	wp_enqueue_script('pleiadesmoon-app', get_theme_file_uri('/assets/js/app.js'), array('jquery'), '20170109', true);
...
// Limit the size of the Excerpt in characters
